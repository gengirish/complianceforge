...
**Integration Points for ComplianceForge:**
- **Project Coding Standards**: Enforce TypeScript strict mode, tRPC patterns, Prisma conventions
...
- **Integration Skills**: GitHub scanner, Stripe billing, Clerk auth integration guides
...
| State Management | Zustand / React Context + Server State (tRPC) | P0 |
...
| Skill | Technologies | Priority |
|-------|-------------|----------|
| API Design | tRPC (type-safe RPC) with Next.js API routes | P0 |
| Database Design | Prisma ORM + PostgreSQL (Neon serverless) | P0 |
| Authentication | Clerk (SSO, MFA, org management) or NextAuth.js | P0 |
| Authorization | Role-based access (Admin, Compliance Officer, Auditor, Viewer) | P0 |
| Background Jobs | BullMQ + Redis for async processing (doc gen, scanning) | P1 |
| File Storage | AWS S3 / Cloudflare R2 for documents and attachments | P1 |
| Caching | Redis (Upstash) for API responses and session data | P1 |
| Rate Limiting | Upstash Redis rate limiter for API and Claude usage | P1 |
...
| API Testing | tRPC test utilities, Supertest | P1 |
...
│               │ API Conventions      │ tRPC, error handling         │
├───────────────┼──────────────────────┼─────────────────────────────┤
...
| **Total unique skills/capabilities** | **98+** | |
